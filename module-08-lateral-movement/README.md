...
> **Evidence note.** The `.evtx` are **real public attack captures** from other hosts (EVTX-ATTACK-SAMPLES), so the embedded host/service names you'll read in the output (e.g. `spoolfool`, `\PSEXESVC`, RDP source IPs) are ground truth, shown **unaltered**.
...
**The mechanism:** **DCOM** (Distributed COM) lets one machine create and drive a COM *object* on another machine. Some built-in COM objects can be coerced into running a command — `MMC20.Application`, `ShellWindows`, `ShellBrowserWindow`, `Excel.Application`, or `mshta`. When abused remotely, the object's host process (`mmc.exe`, `mshta.exe`, an Office app) runs the attacker's payload — and the parent it runs under **depends on the DCOM object**: `MMC20.Application` (`mmc.exe`) and LethalHTA (`mshta.exe`) are **parented by `svchost.exe -k DcomLaunch`** (the service that brokers DCOM), whereas `ShellWindows` / `ShellBrowserWindow` run under an **already-running `explorer.exe`** instead. That the parent *differs by DCOM object* is the giveaway.
...
```
This file is **4 ×** System **10016, "The application-specific permission settings do not grant... DistributedCOM permission."** 10016 fires when a DCOM activation is *denied*. **Failed DCOM lateral movement still leaves 10016 events**, even when the payload never ran. A cluster of 10016 referencing unusual CLSIDs (the GUIDs that identify COM objects) is itself a DCOM-lateral lead — you can catch the *attempt*, not just the success. **Caveat:** 10016 (a DistributedCOM permission denial) has a very large benign baseline — Microsoft documents these as routine and generally ignorable — so on its own it is at best a *weak corroborating* signal, not a standalone lead; the 4 events in this sample are fine to teach on, but don't generalise 10016 into a reliable indicator.
...
> **The authentication *protocol* is a third discriminator.** A 4624 records more than the logon type — its **`AuthenticationPackageName`** field says *which protocol* proved the credential, an axis orthogonal to the parent process. In a healthy domain, machine-to-machine access is overwhelmingly **Kerberos**, which leaves a ticket trail on the **DC** (4768 = TGT requested, 4769 = service ticket) and *no* 4776. **NTLM** is the fallback Windows uses only when Kerberos can't be — most tellingly when a tool connects **by IP** (Kerberos needs an SPN it can only resolve from a name) or hits a **local** account — and every NTLM validation writes a **4776** on the host authoritative for that account. Because **Pass-the-Hash replays an NT hash, an NTLM primitive**, a PtH hop shows up as **4624 Type 3 + AuthenticationPackage = NTLM + a 4776**, in an environment where you'd expect Kerberos: an admin Type-3 that authenticated with NTLM to a machine *by IP, off-hours* is a far sharper lead than "a Type 3 happened." (Field cross-checks: a real NTLM 4624 has non-zero `Key Length` and a populated `Package Name (NTLM only)`, both blank for Kerberos.)
>
> **Same NTLM logon, two stories — PtH vs relay.** Not every NTLM Type-3 hop is Pass-the-Hash: **NTLM relay** produces a nearly identical 4624. In PtH the attacker *holds* the hash and authenticates from a host they control, so a **4648** typically fires on that source. In a relay they never possess the credential — they coerce a victim and *forward* the authentication — so there is **no 4648 on any source you own**, and the tell is a **mismatch inside the target's 4624**: the account belongs to one machine while the **Source Network Address / Workstation Name is a different, unexpected host** (the relay). NTLM Type-3 + a matching source-side 4648 = hands-on PtH/over-pass; NTLM Type-3 with the account's name but the *wrong* origin IP and *no* 4648 = a relayed authentication — same event, different origin story, and two different remediations.
...
| **Sysmon 1** process | parent matches the child's normal launcher | parent = `services.exe` (service-spawned shell) or `svchost.exe -k DcomLaunch` (DCOM) for `cmd`/`mshta`/`mmc` — but `explorer.exe` for the ShellWindows/ShellBrowserWindow DCOM objects |
...
2. **DCOM line-up.** Run Chainsaw on the three DCOM samples. Match each to its COM object (`MMC20.Application`, `ShellWindows`/`ShellBrowserWindow`, `mshta`/LethalHTA) and note that the parent **differs by object**: `svchost.exe -k DcomLaunch` for MMC20 and LethalHTA, but `explorer.exe` for ShellWindows/ShellBrowserWindow — that difference by DCOM object is the lesson. Then open `LM_dcom_..._10016.evtx` — why are the *failed* activations only visible as System **10016**, and why is a cluster of 10016 still a useful lead?
