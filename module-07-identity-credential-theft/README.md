...
> **Evidence note.** The samples are **real public credential-theft captures** from other hosts (`PC04.example.corp`, `IEWIN7`, `DC1.insecurebank.local`, `IEUser`), and those baked-in names are shown in tool output **exactly as recorded** — we never alter evidence.
...
1. **Catch the dump.** With **Sysmon** installed, opening another process's memory is logged as **Sysmon Event ID 10 (ProcessAccess)**. When the *target* of that access is `lsass.exe`, you look at the **`GrantedAccess`** field — the exact memory-access rights that were requested. Certain values are a near-perfect fingerprint of dumping tools:
   ...
   >
   > **Know the LSASS protection state before you trust an *absent* fingerprint.** Two defenses change what a "successful dump" even means, and both leave boot-time artifacts. **LSA Protection (RunAsPPL)** runs `lsass.exe` as a Protected Process Light, so a normal-integrity caller's handle request is *stripped* by the kernel and the tell-tale `0x1010`/`0x143a` read is downgraded or denied — the dump just fails. Attackers defeat it by loading a signed vulnerable **kernel driver** (Mimikatz's `mimidrv.sys`, or BYOVD) to clear the protection bit, which surfaces as a **driver-load artifact — System Event 7045 and/or Sysmon Event 6** — right before the LSASS access. Confirm the regime from **System/WinInit Event 12** ("LSASS.exe was started as a protected process with level: 4"): if PPL was on, an absent `0x1010` is a lead to hunt the bypass, not an all-clear. Second, **Credential Guard** isolates the secrets in a VBS trustlet (**`LsaIso.exe`**), so even a textbook LSASS dump yields **no reusable NTLM hash or TGT** — the attacker gets nothing and pivots to DCSync/Kerberoasting. Triage the *state*, not just the mask.
...
> **A third shape — steal the hash without ever reading it.** Not all credential theft dumps memory. In **Kerberoasting** (ATT&CK **T1558.003**) an ordinary domain user just *requests service tickets* for accounts with a Service Principal Name; each ticket is encrypted with the service account's password hash, cracked **offline** — no LSASS access, no replication, so none of the signals above fire. The tell is in the request: **Security Event 4769** records every service-ticket grant, and roasting tools **downgrade the encryption to RC4** — a **Ticket Encryption Type of `0x17`** where a modern domain should show AES (`0x12`/`0x11`) — plus a *burst* of 4769s for many SPNs from one account. The sibling, **AS-REP roasting** (**T1558.004**), hits accounts with Kerberos pre-auth disabled and shows as **Event 4768** (TGT request) with RC4 and pre-auth type 0. (No roasting sample ships with this module — this is context for the native detection, not an exercise file.)
