...
>
> **Beyond time zones: per-host clock *skew*.** Normalizing to UTC only fixes *zone offset* — it assumes each host's clock was itself *correct*, which it often wasn't. Every machine keeps its own real-time clock, and those clocks drift or were never NTP-synced; a workstation whose clock is 90 seconds fast places its event-log rows 90 s off a correctly-clocked host's filesystem rows **even in UTC**. This module's own merge is the live example: the `$MFT` is from the Case-001 disk and the `.evtx` from the MSEDGEWIN10 host — **two machines, two independent clocks** — so sub-second interleaving is only trustworthy *within* a source, not *across* them. On real multi-host evidence, establish each host's clock error first (compare a known-synced event — a domain logon, a Kerberos ticket, a `w32time`/NTP record — against a reference), record the offset, and correct it before reading cross-host adjacency; otherwise the shared clock is precise but not *accurate*.
...
>
> **The clock column is `M` — and A, B carry their own traps.** The `Time` this merger uses for filesystem rows is `LastModified0x10`, i.e. the **M** of a file's four MACB times — *one* of four, and the other three mislead in specific ways. **A (Accessed)** is the most dangerous to trust: since Windows 10 v1803 `NtfsDisableLastAccessUpdate` defaults to *System Managed*, which **disables** last-access updates on any system volume over 128 GiB — essentially every real machine — so a stale Accessed time does **not** mean "someone opened this." **B (Born/created)** isn't guaranteed earliest: **copying** a file sets the new Created time to the copy moment while *preserving* the source's Modified time, so **B later than M** is normal copy behaviour, not necessarily tampering. Read column *order*, and know which times the OS was even maintaining.
