...
>
> **Three more tells beyond the `$SI`-vs-`$FN` split.** The single-record heuristic has independent corroborators worth checking. **(a) PE compile time** — an executable's `IMAGE_FILE_HEADER.TimeDateStamp` (sigcheck's "Link date", exiftool's `TimeStamp`) records when the linker built it; a binary whose `$SI`/`$FN` creation *predates its own compile time* is impossible, so a 2019 `$SI` on a binary linked in 2026 is a stomp — though the PE stamp is itself forgeable and some toolchains zero it, so a match clears nothing. **(b) ShimCache / AppCompatCache** stores the file's `$SI` last-modified time *as it was when the program ran*; stomp the file *after* execution and ShimCache preserves the real (later) modified time while the on-disk `$SI` now claims older — a registry-vs-filesystem mismatch that exposes the backdating. **(c) MFT entry number** — records are allocated in ascending order over the volume's life, so a genuine 2019 OS file carries a *low* entry number while a file dropped during the intrusion gets a *high, recently-allocated* one; a file backdated to 2019 but sitting at an out-of-sequence high MFT entry (next to other incident-window files) contradicts its own timestamp. (fluxsec.red PE-compile-time stomp; Hexacorn PE timestamps; SherlockForensics ShimCache-stores-`$SI`-M; ATT&CK T1070.006.)
...
>
> **ADS is also an *execution* vector, not just a hiding spot.** Because LOLBins accept a stream path, `rundll32.exe "C:\path\file.txt:evil.dll",EntryPoint` runs a DLL living inside `file.txt`'s ADS while process monitoring shows only an ordinary `rundll32`; real campaigns do this — **WastedLocker**, Valak, and BitPaymer write executable content to an ADS and launch from it. The tell is in the **command line**, not the file listing: a `rundll32`/`wscript`/`cmd` argument containing a colon-plus-stream-name (`file:stream`), which is why Sysmon command-line capture matters here. On disk these appear as extra named `$DATA` attributes in `istat`/MFTECmd and as `STREAM_CHANGE`/`NAMED_DATA_EXTEND` in `$UsnJrnl`. Maps to **MITRE ATT&CK T1564.004 (Hide Artifacts: NTFS File Attributes)** — distinct from the timestomp technique (T1070.006) elsewhere in this module. (attack.mitre.org/techniques/T1564/004; SentinelOne WastedLocker ADS analysis.)
...
>
> **The `$LogFile` side of each action.** Where `$UsnJrnl` names an action once at a high level, `$LogFile` records the *several* low-level opcodes NTFS ran to perform it — operation-level ground truth in the minutes-to-hours before it wraps. A **create** is `InitializeFileRecordSegment` on the new `$MFT` record plus `AddIndexEntryAllocation`/`AddIndexEntryRoot` in the parent index (USN `FileCreate`); a **delete** is `DeleteIndexEntryAllocation` + `DeallocateFileRecordSegment` (USN `FileDelete`); a **rename/move** removes the old index entry and adds a new one — matching the USN `RenameOldName`+`RenameNewName` pair, a *move* differing only by a changed parent-MFT reference; a **timestamp change** (the timestomp) is `SetStandardInformation` (USN `BasicInfoChange`); **data writes** are `UpdateNonresidentValue`/`UpdateResidentValue` (USN `DATA_EXTEND`/`_OVERWRITE`/`_TRUNCATION`); an **ADS write** adds a named `$DATA` attribute (USN `StreamChange`/`NamedDataExtend`). These `$LogFile` opcode *names* come from reverse-engineered tooling (Schicht's LogFileParser, NTFS Log Tracker), not Microsoft documentation — treat them as a well-established community model. (github.com/jschicht/LogFileParser; dfir.ru "how the $LogFile works".)
...
- **Pivot on high-value parent directories.** Because every `$J`/`$FN` record carries the **parent MFT reference**, resolve these parents once (via the `$MFT` join) and filter the journal to them instead of reading it whole: **`Windows\System32`** (DLL/service masquerading into a trusted path), **`Windows\Prefetch`** (a `FileCreate` here *is* execution evidence — a `.pf` written means the program ran), **`\AppData\Local\Temp` and `Windows\Temp`** (dropper/archive staging), **`Users\*\Downloads`** (initial-access payloads, often still MotW-tagged), the **Recent LNK folder** (`\AppData\Roaming\Microsoft\Windows\Recent` — what the user opened), and **`$Recycle.Bin\<SID>`** (deletion and anti-forensics). Filtering to these parents surfaces the create/rename/delete story of an intrusion far faster than a full-journal read. (forensics.wiki timeline analysis; ATT&CK T1074 Data Staged.)
