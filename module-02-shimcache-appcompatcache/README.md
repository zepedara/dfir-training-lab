...
> **Evidence note.** This module reads the real Case-001 `SYSTEM` hive, so tool output shows its real baked-in names unaltered — host `DESKTOP-SDN1RPT`, malware `coreupdater.exe`.
...
1. **It builds up in memory while Windows runs**, and is **only written ("flushed") to the `SYSTEM` hive on a clean shutdown/reboot.** Consequence: if you grab a `SYSTEM` hive from a *live* machine, the newest activity may still be **memory-only** and not on disk yet — and a crash or hard power-off can lose it entirely. So the freshest entries you most want may be exactly the ones missing from a live-acquired hive. (This is "The Volatility of ShimCache" from the deck.) **The remedy: read it from RAM.** If you also captured a memory image, Mandiant's Volatility plugin **`shimcachemem`** parses the AppCompatCache straight out of kernel memory (Windows XP SP2 → Windows 10) — giving you the up-to-the-second cache the on-disk hive hasn't flushed yet, often the *only* way to see recent ShimCache on a long-running server you can't reboot. (Out of scope for this on-disk-hive module, but the natural next move whenever you suspect the hive is stale.)
2. **The Windows 10/11 "executed" flag is not trustworthy.** On Win7 it meaningfully meant "this ran"; on Win10 it is **inconsistent** — in our data **247 of 266 rows read `Executed: No` and only 19 read `Yes`**, and the 19 are an odd assortment (AppX package-identity entries and VMware driver installers) rather than the things you know actually ran. That inconsistency is the point: the flag is not simply "always No", it is **unreliable**, which is worse, because a stray `Yes` invites you to over-claim. **Treat ShimCache as proof of *existence/awareness*, never as proof of execution.** Use Prefetch/Amcache for the "did it run" claim.
...
- **CacheEntryPosition** — `0` is the **most recently inserted**. Lower number = more recent. This is your relative timeline. **Neighbor-bracketing:** entries are cached in the order Windows encountered the files, so an entry sits near others inserted close to it *in time* — a timestamp-less unknown binary wedged between two entries you *can* date independently (an Amcache first-run, a known Windows-Update binary, a Prefetch run time) can be bracketed to that same window. Treat it as *inference*, not proof: position orders entries, but only some carry an anchorable timestamp.
...
- **Executed** — on this Win10 hive **247 rows say `No` and 19 say `Yes`** (check it yourself: `awk -F',' 'NR>1{print $5}' shimcache.csv | sort | uniq -c`). The `Yes` rows are AppX package entries and VMware driver installs — not the binaries you know executed. That is the unreliability in action: do **not** conclude the `No` rows never ran, and do **not** treat a `Yes` as proof one did. (On a Win7 hive this column is meaningful — see the contrast box below.)
...
On a **Windows 7** `SYSTEM` hive, the same tool fills the **Executed** column with real `Yes`/`No` values, because Win7's ShimCache format reliably recorded execution. That is the difference the deck calls out: *trust the flag on Win7-era systems, distrust it on Win10/11.* If you ever parse a Win7 hive you'll see rows like `...\TrustedInstaller.exe ... Yes` — on our Win10 hive the same column is *mostly* `No` with 19 scattered `Yes` values that don't correspond to the execution you can prove elsewhere. Always check the detected format line (`Windows10C_11` vs `Windows7x86`) before you trust the flag.
