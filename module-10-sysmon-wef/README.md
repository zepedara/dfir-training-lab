...
> **Evidence note.** The samples are **real public captures** (EVTX-ATTACK-SAMPLES, hayabusa-sample-evtx) from other hosts; their baked-in names are shown in tool output **unaltered**.
...
- **Sysmon enriches as it records.** A Sysmon process-create (Event ID 1) includes the full command line, the file **hashes**, *and the parent process* — so you get the process *tree*, not just isolated launches. That parent→child chain is what catches "Word launched PowerShell." One of those hashes is special: alongside SHA256 (which a one-byte recompile defeats), Sysmon can compute an **IMPHASH** — a fingerprint of the executable's *import table* (the DLLs/APIs it pulls in). Because that layout is a property of the tool's build, not its exact bytes, an IMPHASH tends to stay **stable across recompiles** and is often **shared across a whole malware/tooling family** — so stacking IMPHASH across the fleet (Module 4) can surface "the same builder ran on nine hosts" even when the file hash differs on every one. (Enable via the config's `HashAlgorithms`; treat a hit as a strong lead, since unrelated programs built the same way can collide.)
...
> **The map now runs past 26 (Sysmon 14/15).** The IDs above cover every sample in this lab, but current Sysmon adds three more you'll meet in the field: **27 `FileBlockExecutable`** and **28 `FileBlockShredding`** (Sysmon 14) *actively block* a write instead of only logging it — a preventive tier — and **29 `FileExecutableDetected`** (Sysmon 15) fires when a PE is written. None appear in this module's captures, but knowing they exist explains gaps if you meet a config that uses them. (Source: Microsoft Sysinternals — Sysmon.)

### Tying the IDs together — the `ProcessGuid`
When you reconstruct a "process tree," the field doing the work is **`ProcessGuid`** — a value Sysmon stamps on a process at creation that is *unique across the whole domain* and stays constant for that process's entire life. Almost every later event that process generates — its network connections (3), image loads (7), the LSASS handle it opens (10), the file it drops (11), the DNS name it resolves (22), its termination (5) — carries that **same** `ProcessGuid`. That's your join key: pivot on one `ProcessGuid` to pull a single process's *complete* story out of a noisy log, and walk the tree by matching a child's `ParentProcessGuid` to a parent's `ProcessGuid`. **Do not** correlate on the raw `ProcessId` (PID) instead — Windows recycles PIDs constantly, so on a busy host the same PID belongs to several unrelated processes within one capture, and a PID-based join silently splices two different processes into one false chain. Microsoft added the GUID for exactly this reason: *"correlation of events even when Windows reuses process IDs."*
...
```
(Every command in this module is run **from inside this `data/` folder**. `EvtxECmd` and `hayabusa` are installed natively and already on your `PATH`, so you call them directly by name in Git Bash — no container, no Docker. The VM is kept offline. See Module 8 §3.)
...
```bash
hayabusa csv-timeline -d . -o timeline.csv -w
```
- `hayabusa` — the timeline/triage tool.
- `csv-timeline` — the mode that produces one ranked CSV across everything.
- `-d .` — the **d**irectory of `.evtx` to scan (all six files at once).
- `-o timeline.csv` — the **o**utput CSV (written into the current `data/` folder). Hayabusa writes straight into the current folder; it will not create a missing sub-folder, so don't point `-o` at a directory that doesn't exist yet.
...
```bash
EvtxECmd -f Sysmon_UACME_45.evtx --csv _out --csvf uacme45.csv
```
...
```bash
EvtxECmd -f Sysmon_UACME_63.evtx --csv _out --csvf uacme63.csv
```
...
```bash
EvtxECmd -f sysmon_10_11_lsass_memdump.evtx --csv _out --csvf lsassdump.csv
```
...
```bash
EvtxECmd -f meterpreter_migrate_to_explorer_sysmon_8.evtx --csv _out --csvf migrate.csv
```
...
```bash
EvtxECmd -f Zerologon_DefaultLogging_Security.evtx --csv _out --csvf zl_default.csv
EvtxECmd -f Zerologon_Sysmon.evtx                  --csv _out --csvf zl_sysmon.csv
```
...
> **Blinding the sensor — three tiers, and why the collector wins.** An admin-level attacker has three escalating ways to go dark: (1) **reconfigure** it with a permissive/empty config — Sysmon keeps running but records nothing, and this fires **EID 16**; (2) **stop/uninstall** the service (`sysmon -u`) — this fires **EID 4 (service state → stopped)**, the last thing Sysmon writes; (3) **unload the kernel driver** directly (`fltmc unload SysmonDrv`, the *Shhmon* technique) — the stealthy one: logging simply **ceases and Sysmon emits no event about it**, so you detect it only from *outside* (the host's forwarded stream goes silent; the **System** log records the minifilter unload). There's also a passive blind spot needing no tampering: the popular configs are **public**, so their *exclude* lists are too — an adversary can name a payload to match an excluded image or launch under an excluded parent and never be recorded (a concrete reason to tune your config and set a non-default `DriverName`). All of this is the real argument for **WEF**: because forwarded events land on a **separate collector**, an attacker who owns the endpoint can silence *future* logging but **cannot reach back and edit or delete the copy already shipped** — the collector is the tamper-resistant record, and that off-box copy, not just scale, is why hunters trust `ForwardedEvents`.
