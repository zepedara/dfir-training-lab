...
**Volatility 3** is the open-source **framework for memory forensics**, maintained by the Volatility Foundation. It is a complete rewrite of the older Volatility 2 (which is now **archived and end-of-life** — Python 2 only — so any `vol.py --profile=…` recipe you find online is legacy v2 syntax; Volatility 3 is the maintained line. A naming quirk worth knowing when you install it: the framework is called *Volatility 3*, but its release tags are numbered **2.x**, so “latest 3.x” is a red herring). On the lab VM it is on the `PATH` as **`vol`**. It does three things:
...
```
- **`windows.malfind`** is the **#1 "is there injected malware?" plugin.** It scans every process's *private* memory for regions that are **committed, executable, and not backed by any file on disk**, with **`PAGE_EXECUTE_READWRITE`** (RWX) protection — the signature of **code injection and reflective DLL loading** (and often, though *not always*, process hollowing: classic hollowing can replace a **mapped image** and leave no private RWX region, so `malfind` can miss it — corroborate, as below). *Newer capability:* Volatility 3 now ships a dedicated **`windows.malware.hollowprocesses`** plugin that catches exactly this by diffing the on-disk PE header against the in-memory image — it closes the gap `malfind` leaves on classic hollowing. (Source: volatility3.readthedocs.io.) For each hit it prints a hex + disassembly preview.
>
> **Tool-currency note — the plugin has a new canonical name.** The command above (`windows.malfind`) still runs, but in current Volatility 3 (v2.27+) this plugin was moved into the malware sub-package: its canonical path is now **`windows.malware.malfind`**, and the bare `windows.malfind` is a **deprecated alias** that emits a `FutureWarning` and is slated for removal in the first release after **2026-06-07** (the same relocation applies to `psxview` → `windows.malware.psxview`). Note the module's own Sources link below already points at `.../plugins/windows/malware/malfind.py` — the new location. Use the `windows.malware.` form on any recent build. (Source: volatility3.readthedocs.io plugin listing.)
