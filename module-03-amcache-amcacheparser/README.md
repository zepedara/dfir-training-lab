...
2. Because inventorying is **scheduled**, Amcache is **not a real-time execution log.** A file can appear in Amcache because it merely *exists* on disk — **"inventoried" does not mean "executed."** Use Prefetch for the execution claim; use Amcache for *identity and presence*. **And absence is not exoneration:** a file an attacker drops, runs, and deletes *before the appraiser next fires* may never be inventoried at all, so an empty Amcache result is *inconclusive* — never proof the binary was never present (corroborate with Prefetch, the USN journal, and the MFT). Entries in fact arise **three ways** — files the appraiser *scanned* in monitored locations, executables/drivers *copied* during an install or run, and GUI apps that *actually required a compatibility shim* — and only the last reliably implies execution; the first two merely confirm the bytes were on disk.
...
- Plus device/driver CSVs (`DevicePnps`, **`DriveBinaries`**, `DriverPackages`, `DeviceContainers`, `ShortCuts`) — **not just device noise.** `DriveBinaries` carries one entry per inventoried **kernel driver**, each with its own **SHA1**, path, and code-signing details. Stack those driver hashes against a known-vulnerable-driver list like **LOLDrivers** exactly the way you stack executable hashes: a legitimately-signed-but-exploitable driver in an odd path is the fingerprint of a **BYOVD ("Bring Your Own Vulnerable Driver")** attack — the classic kernel-level EDR-killer (ATT&CK T1068) — and it is *invisible* to the executable-only workflow this module otherwise teaches.
