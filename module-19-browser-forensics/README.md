...
> **Beyond `History`: the Session/Tabs files — what was *open*, not just *visited*.** `History` answers "what pages were *visited*"; it does **not** tell you which tabs were **open on screen** at seizure — and that state lives in an artifact Hindsight does not parse. Under the profile's **`Sessions\`** folder, Chromium writes **session-restore files in the SNSS format** (signature `SNSS`) recording open tabs/windows plus each tab's back/forward list; older builds used `Current Session`/`Current Tabs` + `Last Session`/`Last Tabs`, modern Chrome uses timestamped `Session_<n>`/`Tabs_<n>` snapshots — collect the whole `Sessions\` directory. On a *running* machine this is gold: a tab open to `mail.proton.me` or a half-finished upload proves engagement at capture even if the URL never hit `History`. SNSS is a length-prefixed "pickle" format, not SQLite, so parse it with a dedicated tool (e.g. Chromagnon) and validate output against a known sample, since several SNSS parsers are unmaintained. (Pairs with gotcha ③: these files capture the un-checkpointed mid-session state a crash or live-pull leaves behind.)
...
> **Decode the `transition` value — it's a *packed* 32-bit integer.** The **low byte (`transition & 0xFF`) is the core type**; the high bytes are qualifier flags OR-ed on top, so mask before you compare. Core types: `LINK=0`, `TYPED=1`, `AUTO_BOOKMARK=2`, `AUTO_SUBFRAME=3`, `MANUAL_SUBFRAME=4`, `GENERATED=5`, `AUTO_TOPLEVEL=6`, `FORM_SUBMIT=7`, `RELOAD=8`, `KEYWORD=9`, `KEYWORD_GENERATED=10`. The payoff is the distinction: **`TYPED` (1) means the user deliberately keyed the address** (first-class intent) whereas `LINK` (0) is a click and `AUTO_SUBFRAME` (3) is an embedded resource the user never chose. Qualifier bits carry the rest: `CLIENT_REDIRECT=0x40000000` / `SERVER_REDIRECT=0x80000000` mark a hop the user was *carried through*, and `CHAIN_START=0x10000000` / `CHAIN_END=0x20000000` bracket a redirect chain (asked-for vs landed-on). So `0x40000001` decodes as **TYPED + CLIENT_REDIRECT** — a typed address that bounced onward — a very different sentence than a bare `LINK`. (Values byte-exact from Chromium `page_transition_types.h`.)
