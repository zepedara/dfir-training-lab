...
The single fact that makes this module tractable: **Chromium is everywhere and it's all the same format.** Google **Chrome**, Microsoft **Edge**, **Brave**, **Opera**, **Vivaldi**, and dozens of niche browsers are all built on **Chromium**, and they all store their profile as the **same set of SQLite databases** in the same layout. Learn to read one Chromium profile and you can read all of them — one tool, one technique, most of the browser market. (Firefox uses a different schema — `places.sqlite` — but the *approach* is identical.)
...
- **Hindsight — repo, releases, and User Guide** — Ryan Benson / Obsidian Forensics: <https://github.com/obsidianforensics/hindsight> (README + wiki cover the plugins, output formats, and the profile databases it parses). Apache-2.0.
- **Ryan Benson's research on Chromium timestamps and history internals** (Obsidian Forensics blog / "Google Analytic Cookies" and Chrome-epoch write-ups) — the authoritative explanation of the 1601 microsecond epoch and the `visits`/`urls` schema.
- **SANS FOR500 — Windows Forensic Analysis** — the **Browser Forensics** section (Chrome/Edge history, downloads, cookies, autofill, and the SQLite/timestamp mechanics), plus the SANS browser-forensics cheat-sheet poster for artifact locations.
- **13Cubed — "Investigating Web Browser Activity" / Chrome forensics episodes** (YouTube) — clear, hands-on walkthroughs of the `History` database, WAL behavior, and timestamp conversion using the same structures shown here.
- **Magnet Forensics & Foxton/Nirsoft field write-ups** — practical notes on DPAPI-encrypted Chrome credentials/cookies and on WAL-mode pitfalls when collecting live.
- **MITRE ATT&CK** (investigative context): **T1071.001 (Application Layer Protocol: Web Protocols)** and **T1567 (Exfiltration Over Web Service)** — web-based C2 and exfil that browser history and download records help evidence; **T1539 (Steal Web Session Cookie)** — why the `Cookies` database is a target as well as an artifact: <https://attack.mitre.org/>.
