...
- **`de_unmanagedpowershell_psinject_sysmon_7_8_10.evtx`** — Sysmon **7** (image load) + **8 (CreateRemoteThread) ×82** + **10**. A non-PowerShell process loaded `System.Management.Automation.dll` (Sysmon 7) and then injected **82** remote threads (Sysmon 8) = **unmanaged PowerShell injection** (PSInject / `powerpick`-style). Chainsaw: **"CreateRemoteThread API and Powershell"** / remote-thread-creation rules.
