...
> **Evidence note.** The hives are the **real** DFIR-Madness Case 001 evidence, and *we never alter evidence*, so the tool output shows the real baked-in names. In our narrative this domain controller is `MINAS-TIRITH-DC01`; its real evidence name — which you'll see throughout the `rip` output, and which we never change — is **`CITADEL-DC01`**. Likewise the desktop stays `DESKTOP-SDN1RPT` (user `mortysmith`) and the malware stays `coreupdater.exe`.
...
> **The caveat that keeps you honest about LastWrite.** A key's LastWrite time is recorded for the **key as a whole, not for each value inside it** — one timestamp per key, marking the *most recent* change to anything under it. So when a Run key holds four values (Step 7), that single LastWrite says only that *something* in the key changed then; it does **not** prove the malicious value in particular was written at that instant, nor when the others arrived — only that they were written at or before it. Report a value's timing as *bounded by* the key's LastWrite, never *pinned to* it, and corroborate the real write moment against another artifact (Prefetch, event logs, BAM). (Harlan Carvey's long-standing caution: the registry records "only one timestamp — the last modification time — per key.")
...
2. **One plugin = one artifact.** `services` lists services, `userassist` decodes UserAssist, `usbstor` lists USB drives, `shellbags` rebuilds folder-browsing history. There are **over 250 plugins** in this build (run `rip -l` to count yours); you don't have to remember a single key path.
...
> **Note on the `.LOG` files.** Each hive ships with its transaction logs (`SYSTEM.LOG1`, `UsrClass.dat.LOG1`, …) — the not-yet-flushed most-recent changes. **RegRipper 3.0 does *not* replay them automatically** (it only warns if a hive is "dirty"). For this case the base hives already contain the evidence; when last-second precision matters, replay logs first with Eric Zimmerman's `rla.exe` or `yarp`/`registryFlush.py`, then re-run RegRipper. We collect the logs regardless, because you always collect them with the hive. **Transaction logs also recover what an attacker *deleted*.** Log replay isn't only about last-second precision — it's anti-anti-forensics: because Windows flushes lazily (dirty cells appended to `.LOG1/.LOG2` while the primary hive updates later), a key or value an attacker *removed* to hide persistence often survives in the log or the hive's own unallocated space. `rla.exe` replays the logs to emit a clean current hive; Maxim Suhanov's `yarp-print --deleted <hive>` goes the other way and carves **deleted** keys/values. If a `services`/`run` sweep looks suspiciously clean for a host you *know* was compromised, that's how you check whether a foothold was deleted rather than never present.
...
- Maxim Suhanov (dfir.ru) — **BAM/DAM** internals (key path, FILETIME, 7-day window): <https://dfir.ru/2020/04/08/bam-internals/>.
