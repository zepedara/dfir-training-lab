...
> This is the **full course index and study guide**. The repository's [`README.md`](README.md) is the quick-start landing page; this file is the deeper walk-through of *what DFIR is*, *how the modules fit together*, and *how to actually run the lab*. New here? Read this top-to-bottom once, then work the modules in order.

> **One themed story (Middle-earth).** The lab is narrated as the breach of **Middle-earth Holdings** ("the realm") by **SAURON / APT-MORDOR** ("the Eye") — a phish to an exec, an insider's host, then total domain compromise of the DC. That theme is applied consistently to the framing, the synthetic Module-04 fleet, and the jokes; it is the lens, not the forensics. Where a module reads **real** evidence (DFIR-Madness Case 001) or a **real** public attack capture, the ground-truth names baked into the bytes are **never changed** — we only map them to the theme in narration (the canonical mapping, host roster, SAURON toolkit, and the evidence-integrity rule live in **[`THEME-MIDDLE-EARTH.md`](THEME-MIDDLE-EARTH.md)**).
...
Two big arcs run through the core modules:
...
14. **[Module 14 — Malicious documents](module-14-malicious-documents)** · tools: `oletools` + the Didier Stevens suite
