...
>
> **The port can lie — dissect by behaviour, not by number.** One caveat sits under `io,phs`: **tshark guesses protocol from the port**. It labels whatever is on TCP 80 "http" and 443 "tls" — exactly the assumption a competent intruder breaks (C2 on **443 to blend in**, an HTTP panel on **8443/8888**), so a port-only reading of the hierarchy sails right past it. Two defences: (1) **force the dissector** with **`-d`** ("Decode As") — e.g. `-d tcp.port==8443,http` re-reads the capture treating 8443 as HTTP, and if real `http.host`/`http.request.uri` fields appear you've confirmed a service hiding on a non-standard port; (2) ask tshark what it *actually* dissected, not what the port implies, by printing `-T fields -e tcp.port -e frame.protocols` — the `frame.protocols` column is the real dissected chain (`eth:ethertype:ip:tcp:http`), and a row where the **port and protocol-chain disagree** (TLS on 80, plain HTTP on 443, SSH on 53) is a protocol-anomaly hit. Port is a hint, not a verdict. Maps to ATT&CK **T1571 (Non-Standard Port)**.
...
### 4f. Measuring a beacon — the interval, not just the talker
§ 4d spots a *repeat talker*; here is how you prove it's a **beacon**. A human session has ragged timing; a beacon fires on a timer, so the **inter-arrival gap between its connection attempts is nearly constant**. Isolate the connection *initiations* — the SYNs — with the display filter `-Y 'tcp.flags.syn==1 && tcp.flags.ack==0'` and let tshark print the gap from the previous displayed packet: `-T fields -e frame.time_epoch -e frame.time_delta_displayed -e ip.dst`. That `frame.time_delta_displayed` column is the seconds since the previous *displayed* (i.e. previous SYN) packet — real C2 shows a column of near-identical deltas (60.02, 59.98, 60.11), and the tiny spread **is the jitter**: a low jitter-to-interval ratio is the mathematical fingerprint of an automated callback, and it holds even when every connection is byte-for-byte encrypted. For the fleet version — bytes/connections bucketed into fixed time windows so a slow beacon shows up as a flat, evenly-spaced row — add `-q -z io,stat,60` (any interval), which tabulates frames and bytes per N-second bin. (Our 8-packet teaching capture has one connection and no interval to measure — the value here is the *method* you carry to a real capture.) Maps to ATT&CK **T1071**.
...
- **malware-traffic-analysis.net** (Brad Duncan) — a large public library of real, labelled malware pcaps to practise this exact triage against once you outgrow the 8-packet lab file: <https://www.malware-traffic-analysis.net/>.
